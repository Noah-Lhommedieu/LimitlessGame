...
public class Function
{
  ...
  public boolean invoctantaIfPossible(Fight fight, Fighter fighter)
	{
		...
		boolean tersiaire = false;
		boolean quaternaire = false;
		
		for (Fighter f : fight.getFighters2(2))
		{
			if (f.getMob() != null)
			{
				if (f.isInvocation())
				{
					if (f.getMob().getTemplate().getId() == 424)//primaire
						primaire = true;
					else if (f.getMob().getTemplate().getId() == 1092)//secondaire
						secondaire = true;
					else if (f.getMob().getTemplate().getId() == 1091)//tersiaire
						tersiaire = true;
					else if (f.getMob().getTemplate().getId() == 1090)//quaternaire
						quaternaire = true;
				}
			}
		}
		if (fighter.haveState(36) == true && !quaternaire)
		{
			spell=Main.world.getSort(1110).getStatsByLevel(5);
		      fighter.setState(36,0,fighter.getId());
		}
		if (fighter.haveState(37) == true && !tersiaire)
		{
			 spell=Main.world.getSort(1109).getStatsByLevel(5);
		      fighter.setState(37,0,fighter.getId());
		}
		if (fighter.haveState(38) == true && !secondaire)
		{
			spell=Main.world.getSort(1108).getStatsByLevel(5);
		      fighter.setState(38,0,fighter.getId());
		}
		if (fighter.haveState(35) == true && !primaire)
		{
  ...
  public boolean buffIfPossibleKrala(Fight fight, Fighter fighter, Fighter target)
  {
    ...
    if(primaire&&secondaire&&tersiaire&&quaternaire )
    {
  ...
  public boolean HealIfPossiblefriend(Fight fight, Fighter f, Fighter target)//boolean pour choisir entre auto-soin ou soin alli�
  {
    ...
    for(Fighter F : fight.getFighters(3))
    {
      ...
      {
    	  double PDVPER=(F.getPdv()*100)/F.getPdvMax();
  ...
  public boolean invocIfPossibleloin(Fight fight, Fighter fighter, List<SortStats> Spelllist)
  {
    ...
    if(fighter.getMob() != null && fighter.getMob().getTemplate() != null
    	&& fighter.getMob().getTemplate().getId() == 42) {
    	if(fighter.nbInvocation() == 4)
    		 return false;
    }else
    if(fighter.nbInvocation()>=fighter.getTotalStats().getEffect(Constant.STATS_ADD_SUM))
  ...
  public int HealIfPossible(Fight fight, Fighter f, boolean autoSoin, double PDVPERmin)//boolean pour choisir entre auto-soin ou soin alli�
  {
    if(fight==null||f==null)
      return 0;
    if(f.isDead())
      return 0;
    if(autoSoin&&(f.getPdv()*100)/f.getPdvMax()>99)
      return 0;
    Fighter target=null;
    SortStats SS=null;
    if(autoSoin)
  ...
  {
    if(fight==null||F==null || Spelllist == null)
      return null;
    int infl=-1500000;
    SortStats ss=null;
    for(SortStats SS : Spelllist)
  ...
  public int attackIfPossibleglyph(Fight fight, Fighter fighter, Fighter f, List<SortStats> Spell)// 0 = Rien, 5 = EC, 666 = NULL, 10 = SpellNull ou ActionEnCour ou Can'tCastSpell, 0 = AttaqueOK
  {
    ...
    for(SortStats S : Spell)
    {
      if(PathFinding.casesAreInSameLine(fight.getMap(),fighter.getCell().getId(),f.getCell().getId(),'z',70))
      {
        ...
        cell=(int) PathFinding.newCaseAfterPush(fight,fighter.getCell(),f.getCell(),-1,false);
    ...
    for(SortStats S : Spell)
    {
      ...
      if(S.getSpellID()==977)
        for(Map.Entry<Integer, Fighter> f : ennemyList.entrySet())
          if(f.getValue()!=null)
            if(!f.getValue().isInvocation() && notcac)
  ...
  public long calculInfluence(SpellEffect SE, Fighter C, Fighter T)
  {
    long inf=0;
    switch(SE.getEffectID())
    {
