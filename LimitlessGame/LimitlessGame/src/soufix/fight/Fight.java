...
public class Fight
{
  ...
  private boolean QUATRE=false;
  private boolean CINQ=false;
  ...
  public void startFight()
  {
    ...
    if(this.collector!=null&&!this.collectorProtect)
    {
      ...
      for(Player player : protectors)
      {
        ...
        {
          SocketManager.GAME_SEND_MESSAGE(player,"Vous ne pouviez pas rejoindre le combat des percepteurs parce que vous n'�tiez pas disponible.");
    ...
    for(Fighter F : getFighters(3))
    {
      ...
      if(player!=null)
      {
    	 /*if(player.getAccount().getSubscribeRemaining() != 0L)
       	SocketManager.GAME_SEND_MESSAGE(player,"Bonus VIP 25% activ�.","008000");*/
    	if(MONO)
    	SocketManager.GAME_SEND_MESSAGE(player,"Aucun bonus exp en solo activ�.","008000");
    	if(DUO)
        SocketManager.GAME_SEND_MESSAGE(player,"Bonus Duo + 5% exp.","008000");
    	if(TROIS)
    	SocketManager.GAME_SEND_MESSAGE(player,"Bonus Trio + 10% exp.","008000");	
    	if(QUATRE)
        SocketManager.GAME_SEND_MESSAGE(player,"Bonus Quatuor + 15% exp.","008000");
    	if(CINQ)
        SocketManager.GAME_SEND_MESSAGE(player,"Bonus Quintuor + 20% exp.","008000");
    	if(SIX)
        SocketManager.GAME_SEND_MESSAGE(player,"Bonus Sextuor + 25% exp.","008000");
    	if(SEPT)
        SocketManager.GAME_SEND_MESSAGE(player,"Bonus Septuor + 30% exp.","008000");
    	if(HUIT)
        SocketManager.GAME_SEND_MESSAGE(player,"Bonus Octuor + 35% exp.","008000");
    	if(PLUSQUEHUIT)
        SocketManager.GAME_SEND_MESSAGE(player,"Bonus + de 8 Joueurs + 45% exp.","008000");
      }
      }
      if(player!=null)
    	  player.itemchek();
              ...
              {// Soit il a lancer le combat => annulation du combat
                for(Fighter fighter : this.getFighters(caster.getTeam2()))
                {
                  ...
                  {// Celui qui a fait le fight meurt + perte honor
                    ...
                    {
                      int looseEnergy=Formulas.getLoosEnergy(player.getLevel(),getType()==1,getType()==5),totalEnergy=player.getEnergy();
                      ...
                      if(player.isOnline())
                        SocketManager.GAME_SEND_Im_PACKET(player,"034;");
...
public void Anti_bug () {
    ...
    if(!buffsToAdd.isEmpty())
    {
      for(Pair<Fighter, ArrayList<SpellEffect>> pair : buffsToAdd)
      {
        ...
        for(SpellEffect buff : buffs)
          fighter.addBuffTooltip(buff.getEffectID(),(int) buff.getValue(),buff.getDuration(),buff.getTurn(),buff.isDebuffable(),buff.getSpell(),buff.getArgs(),fighter,true);
      }
      buffsToAdd.clear();
    }
  ...
  public void joinFight(Player perso, int guid)
  {
    ...
    if(getTeam0().containsKey(guid))
    {
      ...
      if(getType()==Constant.FIGHT_TYPE_AGRESSION)
      {
        ...
        if(multiIp)
        {
          SocketManager.GAME_SEND_MESSAGE(perso,"Vous ne pouvez pas rejoindre ce combat, il y a D�j� un personnage dans ce combat avec la m�me IP.");
          return;
        }
      }
      if(getType()==Constant.FIGHT_TYPE_PVT)
    ...
    else
    {
     ...
     // window
        ...
        if(arme.isHealWeapon())
        {
          ...
          {
            double heal=Formulas.getRandomValue(heals.getLeft(),heals.getRight());
            if(isCC)
 ...
 // Valeur original de isEc = Formulas.isCriticalFail(spell.getTauxEC(),fighter)
      boolean isEc= false;
      if(isEc)
        SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,302,fighter.getId()+"",spell.getSpellID()+""); // envoi de  l'EC
          ...
          if(player!=null)
          {
            if(this.isBegin())
            {
              ...
              if(player.getParty() != null && player.getParty().getMaster() != null &&  player.getParty().getMaster().getId() != player.getId())
              {
            	  ...
            	  TimerWaiterPlus.addNext(() -> {
            		  player.send("GE");
            		  //player.lastfight = packet; � rajouter ap�s
 				 },500);
 ...
 if(player.getMetiers().size()>0)
 {
    ...
    if(this.getType()==Constant.FIGHT_TYPE_KOLI) {
    	TimerWaiterPlus.addNext(() ->  player.teleportOld(),6000);
    	 if(Main.world.addKoli_players(player))
      	  { 
      	   SocketManager.GAME_SEND_MESSAGE(player,"Vous vous �tes inscrit au Kolizeum", "008000");
      	  }
  ...
  //ONE WINDOWS
    ...
    if(this.getType()==Constant.FIGHT_TYPE_KOLI) {
    	TimerWaiterPlus.addNext(() ->  player.teleportOld(),6000);
    	 SocketManager.GAME_SEND_MESSAGE(player,"Vous avez quitt� le Kolizeum", "FF0000");
    ...
    try
    {
          ...
          for(int i=0;i<this.getCapturer().size();i++)
          {
            try
            {
              ...
              if(f!=null&&f.getPersonnage()!=null)
              {
                ...
                Pair<Integer, Integer> playerSoulStone=Formulas.decompPierreAme(f.getPersonnage().getObjetByPos(Constant.ITEM_POS_ARME));// Rï¿½cupï¿½re les stats de la pierre ï¿½quippï¿½
      ...
      /*  int totalLoserRank=0;
        ...
        if(isAlone)
        {
          for(Fighter fighter : loosers)
          {
            ...
            {
              SocketManager.GAME_SEND_MESSAGE(curPlayer,"Thomas Sacre : Le contrat a �t� ex�cut�, venez me voir pour r�clamer votre r�compense.","000000");
     ...
     /* if(this.getType()==Constant.FIGHT_TYPE_CONQUETE)
      ...
      if(Config.getInstance().HEROIC && this.getMapOld().getSong() != 1)
      {
        switch(this.getType())
        {
          ...
          case Constant.FIGHT_TYPE_AGRESSION:
            ...
            double xp_taux = xp/winners.size();
      ...
      for(Fighter i : winners)
      {
        ...
        {
          String drops="";
          double xpPlayer=0,xpGuild=0,xpMount=0,xpOmega=0;
          int winKamas;

          AtomicReference<Double> XP=new AtomicReference<>();
          AtomicReference<Double> xpOmegaAR = new AtomicReference<>();
          ...
          if(player!=null)
          {
        	  if (DUO)
              {
              	xpPlayer = xpPlayer * 1.05;
              	xpOmega = xpOmega * 1.15;
              }
              if (TROIS)
              {
              	xpPlayer = xpPlayer * 1.1;
              	xpOmega = xpOmega * 1.25;
              }
              if (QUATRE)
              {
              	xpPlayer = xpPlayer * 1.15;
              	xpOmega = xpOmega * 1.35;
              }
              if (CINQ)
              {
              	xpPlayer = xpPlayer * 1.2;
              	xpOmega = xpOmega * 1.45;
              }
              if (SIX)
              {
              	xpPlayer = xpPlayer * 1.25;
              	xpOmega = xpOmega * 1.55;
              }
              if (SEPT)
              {
              	xpPlayer = xpPlayer * 1.3;
              	xpOmega = xpOmega * 1.6;
              }
              if (HUIT)
              {
              	xpPlayer = xpPlayer * 1.35;
              	xpOmega = xpOmega * 2.1;
              }
              if (PLUSQUEHUIT)
              {
        	  ...
        	  if (player.getLevel() == 8000 && player.getPrestige() == 50)
        	  
          {
              xpOmega =  Formulas.getXp(i,winners,totalXP,nbbonus,this.getMobGroup()!=null ? getMobGroup().getStarBonus(getMobGroup().getInternalStarBonus()) : 0,challXp,lvlMax,lvlMin,lvlLoosers,lvlWinners,Main.world.getConquestBonus(player), player);
              xpOmegaAR.set((double) player.xpOmega);
              player.setxpOmega(xpOmega);
              Database.getStatics().getPlayerData().updateXpOmega(player);
          }
            xpPlayer=Formulas.getXp(i,winners,totalXP,nbbonus,this.getMobGroup()!=null ? getMobGroup().getStarBonus(getMobGroup().getInternalStarBonus()) : 0,challXp,lvlMax,lvlMin,lvlLoosers,lvlWinners,Main.world.getConquestBonus(player), player);
            xpPlayer = Math.round(xpPlayer);
          ...
          if(player!=null||(i.getMob()!=null&&i.getMob().getTemplate().getId()==285))
          {
            ...
            for(Entry<Integer, Integer> entry : objectsWon.entrySet())
            {
              ...
              SocketManager.GAME_SEND_Im_PACKET_TO_ALL("116;"+"<b>Server</b>"+"~ON dirait que la chance sourit � ["+playerTarget.getName()+"] Il vient de drop ["+objectTemplate.getName()+"]");
              }
            ...
            if(player!=null)
            {
                ...
                if(player.getMorphMode())
                {
                  ...
                  if(obj!=null)
                    if(Constant.isIncarnationWeapon(obj.getTemplate().getId()))
                      if(player.addXpIncarnations((long) xpPlayer))
                        i.setLevelUp(true);
                }
              ...
              if(xpGuild>0&&player.getGuildMember()!=null)
                player.getGuildMember().giveXpToGuild((long) xpGuild);
            }
      ...
      if(Collector.getCollectorByMapId(getMap().getId())!=null&&getType()==Constant.FIGHT_TYPE_PVM)
      {
        ...
        double winxp=Formulas.getXp(collector,winners,totalXP,nbbonus,(getMobGroup()!=null ? getMobGroup().getStarBonus(getMobGroup().getInternalStarBonus()) : 0),challXp,lvlMax,lvlMin,lvlLoosers,lvlWinners,0, curPlayer)/15;
