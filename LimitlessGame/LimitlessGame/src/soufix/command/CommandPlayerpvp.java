		...
		if (msg.length() > 3 && msg.substring(1, 4).equalsIgnoreCase("all") || msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("monde")) {
			...
			if (msg.length() > 3 && msg.substring(1, 4).equalsIgnoreCase("vip")) {
				SocketManager.PACKET_POPUP_DEPART(perso, 
						 "\n- Vos points acquis par vote sur Serveur-Prive passent à 20 PB par vote."
						+ "\n+ <b>10.000</b> pods de plus."
						+ "\n<b>.aura</b> - Aura exlusive multicolore"
						+ "\n Vous obtenez un bonus de 15% point d'honneur  à chaque combat pvp");
				return true;
			}
			...
			{
				SocketManager.GAME_SEND_MESSAGE(perso,"<b>Liste (Faites .exopa + l'item (à porter sur le personnage) que vous voulez exo +1PA) : <br /></b> Coiffe, Cape, AnneauDroite, AnneauGauche, Amulette, Ceinture, Bottes , Cac", "222222");
			    return true;
			}					    
		    ...
		    {
		    	SocketManager.GAME_SEND_MESSAGE(perso,"<b>Liste (Faites .exopa + l'item (à porter sur le personnage) que vous voulez exo +1PA) : <br /></b> Coiffe, Cape, AnneauDroite, AnneauGauche, Amulette, Ceinture, Bottes", "222222");
				return true;
			}
			...
			{
				SocketManager.GAME_SEND_MESSAGE(perso,"Ton item te donne déjà 1 PM.","222222");
				return true;
			}			
			if(stats.getEffect(111) > 0)
			{
				SocketManager.GAME_SEND_MESSAGE(perso,"Ton item te donne déjà 1 PA.", "222222");
				return true;
			}
			...
			else 
			{
					...
					SocketManager.GAME_SEND_SPELL_LIST(perso);
					SocketManager.GAME_SEND_MESSAGE(perso,"<b>Succès !</b> Votre "+ ((GameObject) items).getTemplate().getName()+ " donne désormais +1PA en plus de ses jets habituels ! ","#01D758");
			...
			{
				SocketManager.GAME_SEND_MESSAGE(perso,"<b>Liste (Faites .exopm + l'item (à porter sur le personnage) que vous voulez exo +1PM) : <br /></b> Coiffe, Cape, AnneauDroite, AnneauGauche, Amulette, Ceinture, Bottes , Cac", "222222");
			    return true;
			}					    
			...
			else 
			{
					...
					SocketManager.GAME_SEND_ASK(perso.getGameClient(), perso);
					SocketManager.GAME_SEND_SPELL_LIST(perso);
					SocketManager.GAME_SEND_MESSAGE(perso,"<b>Succès !</b> Votre "+ ((GameObject) items).getTemplate().getName()+ " donne désormais +1PM en plus de ses jets habituels !","#01D758");
			...
			if (msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("fmcac")) {
				...
				for (int i = 0; i < obj.getEffects().size(); i++) {
				 ...
				 perso.sendMessage("Votre objet : " + obj.getTemplate().getName() + " a été FM avec succès en " + answer);
				return true;
		}
