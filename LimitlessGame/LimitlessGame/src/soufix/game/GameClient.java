...
//anti flodd out
  ...
  public void parsePacket(String packet) throws InterruptedException
  {
    ...
    if (Config.getInstance().debugComplet)
    {
    	if (this.getPlayer() != null)
    		System.out.println("Serv received from " + this.getPlayer().getName() + " : " + packet);
    	else 
    		System.out.println("Serv received : " + packet);	
    }
   /* if(this.player!=null)
    {
      ...
      case 'w':
  	  ...
  	  case 'y':
          ...
          int ornement = Integer.parseInt(packet.substring(2));
                  ...
                  if(World.getOrnements().get(ornement) == null) {
                      player.sendMessage("Une erreur est survenue, merci de contacter un membre de staff.");
                      break;
                  }
                  if(player.getOrnementsList().contains(ornement))
                      player.setOrnement(ornement);
                  else {
                      if(player.getAccount().getPoints() < World.getOrnements().get(ornement).getPrice()) {
                          player.sendMessage("Vous n'avez pas assez de points boutiques pour acheter cet ornement !");
                          break;
                      } else {
                          player.setOrnement(ornement);
                          player.addOrnements(ornement);
                          player.getAccount().setPoints(player.getAccount().getPoints() - World.getOrnements().get(ornement).getPrice());
  ...
  private void parseorn() {
		
		player.send("km3,0,0;6,0,0;9,0,0;11,0,0;13,0,0;15,0,0;18,0,0;21,0,0;24,0,0;25,0,0;26,0,0;28,0,0;29,0,0;30,0,0;31,0,0;32,0,0;51,0,0;58,0,0;85,0,0;87,0,0;88,0,0;89,0,0;108,0,0;109,0,0;111,0,0;112,0,0;114,0,0;115,0,0;117,0,0;118,0,0;120,0,0;121,0,0;123,0,0;124,0,0;126,0,0;127,0,0;128,0,0;129,0,0;130,0,0;131,0,0;132,0,0;135,0,0;136,0,0;139,0,0;140,0,0;141,0,0;142,0,0;146,0,0;147,0,0;148,0,0;149,0,0;150,0,0;151,0,0;152,0,0;153,0,0;154,0,0;155,0,0;156,0,0;157,0,0;158,0,0;159,0,0;160,0,0;161,0,0;162,0,0;163,0,0;164,0,0;165,0,0;166,0,0;167,0,0;168,0,0;169,0,0;170,0,0;171,0,0;172,0,0;173,0,0;175,0,0;176,0,0;177,0,0;179,0,0;180,0,0;181,0,0;182,0,0;183,0,0;184,0,0;185,0,0;186,0,0;187,0,0;188,0,0");
	}
  /**
   * AccountPacket *
   */
	...
	private int objeto_Desequipar_Set() {
		try {
			...
			byte[] orden = {Constant.ITEM_POS_DOFUS1, Constant.ITEM_POS_DOFUS2, Constant.ITEM_POS_DOFUS3,
					Constant.ITEM_POS_DOFUS4, Constant.ITEM_POS_DOFUS5, Constant.ITEM_POS_DOFUS6,Constant.ITEM_POS_DOFUS7, Constant.ITEM_POS_DOFUS8, 
					Constant.ITEM_POS_DOFUS9, Constant.ITEM_POS_DOFUS10, Constant.ITEM_POS_DOFUS11, Constant.ITEM_POS_DOFUS12,Constant.ITEM_POS_DOFUS_ULTIME_1,
...
public void setTimeLastTaverne(long timeLastTaverne) {
        ...
        case 'C':// Certificats -> Etable
          ...
          boolean can = false;
			...
			if(this.player.getId() != mount.getOwner() && !can){
				SocketManager.GAME_SEND_MESSAGE(this.player,"vous tentez d'utiliser une monture qui n'appartient pas à ce personnage !");
			return;	
			}
          mount.setEtape(1);
		  mount.setNumber(1);
  ...
  private void changeMemberProfil(String packet)
  {
    ...
    Player p=Main.world.getPlayer(guid); //Cherche le this.playernnage a qui l'on change les droits dans la mà¯Â¿Â½moire
    ...
    try
    {
      ...
      else
      {
       ...
       /* if(!object.getTemplate().getConditions().contains("PJ"))
        ...
        if(position!=Constant.ITEM_POS_NO_EQUIPED&&(object.getTemplate().getPanoId()!=-1||object.getTemplate().getType()==Constant.ITEM_TYPE_DOFUS||object.getTemplate().getType()==Constant.ITEM_TYPE_DOFUS_ULTIME)&&this.player.hasEquiped(object.getTemplate().getId()))
        {
          return;
        }
...
Logging.getInstance().write("DDOS","IP Same Packet 500 out "+packet+" "+ip);
    ...
    {
    	if (this.getPlayer() != null) 
    		System.out.println("Serv sent to  " + this.getPlayer().getName() + " : " + packet);
    	else 
    		System.out.println("Serv sent : " + packet);
    }
  }
     ...
     SocketManager.GAME_SEND_MESSAGE(this.player,""
    	 ...
    	 SocketManager.GAME_SEND_MESSAGE(this.player,""
    	    		 ...
    	    		 "</b>. Combats PVP gagnés : <b>"+World.get_Succes(this.getPlayer().getId()).getPvp()+ ""
    	    		 		+ "</b>. Quêtes terminées : <b>"+World.get_Succes(this.getPlayer().getId()).getQuete()
    	    		 +"</b>. Donjons effectués : <b>"+World.get_Succes(this.getPlayer().getId()).getDonjon()
...
private void batlle_pass () {
  ...
  public void Koli_add() {
	  ...
	  { 
	   SocketManager.GAME_SEND_MESSAGE(this.player,"Vous vous êtes inscrit au Kolizeum", "008000");
	  }
	  else
	  {
	 SocketManager.GAME_SEND_MESSAGE(this.player,"Vous êtes déjà inscrit au Kolizeum", "FF0000");
  ...
  public void Bourse_add (String packet) {
	  ...
	  leaveExchange(this.player);
		...
		if(have) {
			this.player.sendMessage("Vous possède déjà une offre en cours");
			return;
		}
