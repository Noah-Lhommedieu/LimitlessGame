...
public class Dopeul
{
  ...
  public static void getReward(Player player, int type)
  {
    ...
    switch(type)
    {
      case 1: //Sort spï¿½cial
        ...
        {
          SocketManager.GAME_SEND_MESSAGE(player,"Vous n'êtes pas dans le temple de votre classe.");
          return;
        } else if(player.hasSpell(Constant.getSpecialSpellByClasse(player.getClasse()))) // Si on a dï¿½jï¿½ le sort
        {
          SocketManager.GAME_SEND_MESSAGE(player,"Vous avez déjà appris le sort.");
          return;
        }
      ...
      case 2://Trousseau de clï¿½s
        ...
        {
          SocketManager.GAME_SEND_MESSAGE(player,"Vous avez déjà le trousseau de clés.");
          return;
        }
  ...
  {
    java.util.Map<Integer, Pair<Integer, Integer>> changeDopeul=new HashMap<Integer, Pair<Integer, Integer>>();
