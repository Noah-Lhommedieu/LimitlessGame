  ...
  public Player(int id, String name, int groupe, int sexe, int classe, int color1, int color2, int color3, long kamas, int pts, int _capital, int energy, int level, double d, int _size, int _gfxid, byte alignement, int account, Map<Integer, Integer> stats, byte seeFriend, byte seeAlign, byte seeSeller, String canaux, short map, int cell, String stuff, String storeObjets, String pdvPer, String spells, String savePos, String jobs, int mountXp, int mount, int honor, int deshonor, int alvl, String z, int title, int wifeGuid, String morphMode, String allTitle, String emotes, long prison, boolean isNew, String parcho, long timeDeblo, boolean noall, String deadInformation, byte deathCount, long totalKills, final int tokens, final int apExo, final int mpExo, final int raExo, String rapid,int song, boolean Reload_item, int prestige, int ornement,int omega, double xpOmega,String ListOrnaments)
  {
   ...
   // this.mpExo=mpExo;
    ...
    try
    {
      ...
      this.ornement = ornement;
      try {
          if (!ListOrnaments.isEmpty() || !ListOrnaments.equalsIgnoreCase(""))
              for (String i : ListOrnaments.split(","))
...
//** ORNEMENT **//
  ...
  public static Player CREATE_PERSONNAGE(String name, int sexe, int classe, int color1, int color2, int color3, Account compte, String ListOrnaments)
  {
    ...
    SpellMax(perso);
    perso.getStats().addOneStat(Constant.STATS_ADD_VITA, + 150);
    ...
    /*for (final ObjectTemplate t : Main.world.getItemSet(5).getItemTemplates()) {
    	...
    	/*for (int i=0; i< Config.getInstance().startItem.length;i++) {
            ObjectTemplate t = Main.world.getObjTemplate(Config.getInstance().startItem[i]);
            if(t==null) continue;
            final GameObject obj = t.createNewItem(1, true);
            if (perso.addObjet(obj, true)) {
                World.addGameObject(obj, true);
...
public void set_gameAction_rapide(GameAction _gameAction_rapide) {
  ...
  public void setExp(double totalXp)
  {
    this.exp=totalXp;
  }
  /** ORNEMENT **/
  ...
  public int getPrestige()
  {
    ...
    public double getxpOmega()
    {
    	return this.xpOmega; 
    }
    
    public void setxpOmega(double xpOmega) 
...
public void setTotal_reculte() {
  ...
  public void AddItem(final int id, final int qua) {
      if (qua < 1) {
          return;
      }
      final ObjectTemplate t = World.getObjTemplate(id);
      final GameObject obj = t.createNewItem(qua, false);
  ...
  public void OnJoinGame()
  {
   ...
   // for(int a=1;a<Main.world.getItemSetNumber();a++)
    ...
    if(!Config.getInstance().startMessage.equals("")) //Si le motd est notifiï¿½
    ...
    if(this.time_last_connecte < (System.currentTimeMillis()/1000)){
		...
		double val = (int) ((System.currentTimeMillis()/1000)-this.time_last_connecte);
	...
	int fake = 0;
	 ...
	 if(calcul>1200000) {
       ...
       {
      this.sendMessage("Bienvenue sur <b>Limitless</b>, <b>"+(Main.world.getOnlinePlayers().size()+fake)+"</b> connectes.");
       }
       if(Config.singleton.serverId == 6) 
       {
  ...
  public String getAsPacket(boolean boutique ,boolean window)
  {
    ...
    ASData.append(boutique ? kamas+5000 : kamas).append("|").append(_capital).append("|").append(_spellPts).append("|");
    ...
    double pdv=this.curPdv;
    double pdvMax=this.maxPdv;
    ...
    ASData.append(stats.getEffect(Constant.STATS_ADD_PROS)+sutffStats.getEffect(Constant.STATS_ADD_PROS)+buffStats.getEffect(Constant.STATS_ADD_PROS)).append("|");
  ...
  public Stats newStatsMorph()
  {
    ...
    stats.addOneStat(Constant.STATS_ADD_SUM,15);
    this.useCac=false;
    return stats;
  }
  ...
  public boolean addObjet(GameObject newObj, boolean stackIfSimilar)
  {
    for(Entry<Integer, GameObject> entry : objects.entrySet())
    {
      ...
      {
        obj.setQuantity(obj.getQuantity()+newObj.getQuantity(), this);//On ajoute QUA item a la quantitï¿½ de l'objet existant
        if(isOnline)
  ...
  public boolean levelUp(boolean send, boolean addXp)
  {
    ...
    if (this.getLevel() == 500)
    {
    	...
    	this.getStats().addOneStat(Constant.STATS_ADD_PM, 2);
    }
    if (this.getLevel() == 1000)
    {
    	this.getStats().addOneStat(Constant.STATS_ADD_PA, 2);
    }
    if (this.getLevel() == 2000)
    {
    	this.getStats().addOneStat(Constant.STATS_ADD_PA, 1);
    	this.getStats().addOneStat(Constant.STATS_ADD_PM, 1);
    }
    if (this.getLevel() == 3000)
    {
    	this.getStats().addOneStat(Constant.STATS_ADD_PA, 1);
    }
    if (this.getLevel() == 4000)
    {
    	this.getStats().addOneStat(Constant.STATS_ADD_PA, 1);
    	this.getStats().addOneStat(Constant.STATS_ADD_PM, 1);
    }
    if (this.getLevel() == 5000)
    {
    	this.getStats().addOneStat(Constant.STATS_ADD_PA, 1);
    }
    if (this.getLevel() == 6000)
    {
    	this.getStats().addOneStat(Constant.STATS_ADD_PA, 1);
    	this.getStats().addOneStat(Constant.STATS_ADD_PM, 1);
    }
    if (this.getLevel() == 7000)
    {
    	this.getStats().addOneStat(Constant.STATS_ADD_PA, 1);
    }
  ...
  public void unequipedObjetPrestige() {
  	if(this._onMount)
  		this.toogleOnMount();
  	...
  	GameObject o = this.getObjetByPos(i);
  	if(o == null)continue check;
  ...
  public void teleport(short newMapID, int newCellID)
  {
    ...
    // Verifier la validitï¿½ du Collector
  ...
  public void addInBank(int guid, int qua)
  {
    ...
    if(PersoObj.getTemplate().getType() == 24)
 	   return;
    if(PersoObj.getPosition()!=Constant.ITEM_POS_NO_EQUIPED) // Si c'est un item ï¿½quipï¿½ ...
      return;
    ...
    {
      if(newQua<=0) // Ajout de toute la quantitï¿½ disponible
      {
        removeItem(PersoObj.getGuid()); // On enleve l'objet du sac du joueur
        account.getBank().add(PersoObj); // On met l'objet du sac dans la banque, avec la meme quantitï¿½
      ...
      {
        PersoObj.setQuantity(newQua, this); //on modifie la quantitï¿½ d'item du sac
    ...
    // S'il y avait un item du meme template
    {
      if(newQua<=0) //S'il ne reste pas d'item dans le sac
      {
        ...
        BankObj.setQuantity(BankObj.getQuantity()+PersoObj.getQuantity(), this); //On ajoute la quantitï¿½ a l'objet en banque
  ...
  public void VerifAndChangeItemPlace()
  {
    ...
    for(GameObject obj : objects.values())
    {
      ...
      else if(obj.getPosition()==Constant.ITEM_POS_DOFUS_ULTIME_1)
      {
        if(isFirstDf)
        {
          isFirstDf=false;
  ...
  public void meetWife(Player p)// Se teleporter selon les sacro-saintes autorisations du mariage.
  {
    ...
    if(this.getPodUsed()>=this.getMaxPod()) // Refuser la tï¿½lï¿½portation si on est full pods.
    {
...
public void setOne_windows(boolean one_windows) {
  ...
  public void itemchek (){
		...
		this.chekhackitem(Constant.ITEM_POS_DOFUS_ULTIME_1);
	...
	public static void SpellMax(Player player)
    {
    	int lvlMax = 6;
        boolean changed = false;
        for (Spell.SortStats sort : player.getSorts().values()) {
