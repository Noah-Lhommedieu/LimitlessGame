...
public class House
{
  ...
  public House(int id, short mapId, int cellId, int houseMapId, int houseCellId, int sale_base)
  {
    this.id=id;
    this.mapId=mapId;
    this.cellId=cellId;
  ...
  {
    if(((packet.compareTo(this.getKey())==0))||isHome)//Si c'est chez lui ou que le mot de passe est bon
    {
      P.teleport((short)this.getHouseMapId(),this.getHouseCellId());
      closeCode(P);
    }
    else if((packet.compareTo(this.getKey())!=0))//Mauvais code
    {
  ...
  public static void buy(Player player)//Acheter une maison
  {
    ...
    if(player.getKamas()<house.getSale() || house.getSale() == 0)
      return;
    ...
    for(Trunk trunk : Trunk.getTrunksByHouse(house))
    {
      if(house.getOwnerId()>0)
        trunk.moveTrunkToBank(Main.world.getAccount(house.getOwnerId()));//Dï¿½placement des items vers la banque
    ...
    if(house.getOwnerId()>0)
    {
      ...
      if(seller.getCurrentPlayer()!=null)//FIXME: change the packet (Im)
        SocketManager.GAME_SEND_MESSAGE(seller.getCurrentPlayer(),"Une maison que vous possédiez a été vendue "+house.getSale()+" kamas.");
      //Database.getStatics().getAccountData().update(seller);
    }else {
     Main.world.kamas_total -= house.getSale();	
    }
        ...
        else
        {
          ...
          if(Main.world.getPlayer(house.getValue().getOwnerId())==null)
            packet.append(name+";");
        	  ...
        	  if((Main.world.getPlayer(house.getValue().getOwnerId()).getAccount() != null))
          packet.append(Main.world.getPlayer(house.getValue().getOwnerId()).getAccount().getPseudo()+";");
  ...
  public int getSale_base() {
	return sale_base;
}

public void setSale_base(int sale_base) {
	this.sale_base = sale_base;
}
