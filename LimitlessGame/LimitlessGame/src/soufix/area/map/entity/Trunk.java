...
public class Trunk
{
  ...
  public static void open(Player P, String packet, boolean isTrunk)
  {//Ouvrir un coffre
    Trunk t=(Trunk)P.getExchangeAction().getValue();
    if(t==null)
      return;
  ...
  public void addInTrunk(int guid, int qua, Player P)
  {
    ...
    {
      SocketManager.GAME_SEND_MESSAGE(P,"Le nombre maximum d'objets dans ce coffre a été atteint!");
      return;
    }
    ...
    // S'il y avait un item du meme template
    {
      ...
      if(newQua<=0)
      {
        ...
        TrunkObj.setQuantity(TrunkObj.getQuantity()+PersoObj.getQuantity(),P);
  ...
  public void removeFromTrunk(int guid, int qua, Player P)
  {
    ...
    } else
    {
      ...
      if(newQua<=0)
      {
        ...
        PersoObj.setQuantity(PersoObj.getQuantity()+TrunkObj.getQuantity(),P);
        //On envoie les packets
        SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(P,PersoObj);
        str="O-"+guid;
      } else
