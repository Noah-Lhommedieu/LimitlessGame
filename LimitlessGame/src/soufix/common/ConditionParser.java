...
public class ConditionParser
{
  ...
  private static boolean haveCe(String req, Player player)
  {
    ...
    {
      Pair<Integer, Integer> couple=dopeuls.get((int)map.getId());
      if(couple==null)
        return false;

      int IDmob=couple.getLeft();
  ...
  private static boolean haveNPC(String req, Player perso)
  {
    ...
    {
      case 9052:
        if(perso.getCurCell().getId()==268&&perso.get_orientation()==7)//TODO
          return true;
      case 8905:
    ...
    else
    {
      ...
      if(cond.contains("=="))
      {
        for(String cur : cond.split("=="))
        {
          ...
          if(cur.contains("!="))
            continue;
          if(Integer.parseInt(cur) == 972 || Integer.parseInt(cur) == 17917 ||
					Integer.parseInt(cur) == 8072 || Integer.parseInt(cur) == 17918){
        	  if(perso.hasItemTemplatev2(Integer.parseInt(cur),1))
  ...
  {
	  if(obj.getTemplate() == null || newObj.getTemplate() == null)
		  return false;
    ...
    return obj.getTemplate().getId()==newObj.getTemplate().getId()&&stackIfSimilar&&obj.getStats().isSameStats(newObj.getStats())&&!Constant.isIncarnationWeapon(newObj.getTemplate().getId())&&newObj.getTemplate().getType()!=Constant.ITEM_TYPE_CERTIFICAT_CHANIL&&newObj.getTemplate().getType()!=Constant.ITEM_TYPE_FAMILIER&&newObj.getTemplate().getType()!=Constant.ITEM_TYPE_PIERRE_AME_PLEINE&&newObj.getTemplate().getType()!=Constant.ITEM_TYPE_OBJET_ELEVAGE&&newObj.getTemplate().getType()!=Constant.ITEM_TYPE_CERTIF_MONTURE&&newObj.getTemplate().getType()!=Constant.ITEM_TYPE_OBJET_VIVANT&&(newObj.getTemplate().getType()!=Constant.ITEM_TYPE_QUETES||Constant.isFlacGelee(obj.getTemplate().getId())||Constant.isDoplon(obj.getTemplate().getId()))&&obj.getPosition()==Constant.ITEM_POS_NO_EQUIPED&&obj.getTxtStat().get(Constant.STATS_MIMI_ID)==null&&newObj.getTxtStat().get(Constant.STATS_MIMI_ID)==null;
  }
}
