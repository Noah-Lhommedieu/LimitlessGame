...
public class SpellEffect
{
  ...
  public SpellEffect(long stat, long gain, int aduration, int turns2, boolean debuff, Fighter aCaster, String args2, int aspell)
  {
    effectID=(int) stat;
    value=gain;
    turns=turns2;
  ...
  public static long applyOnHitBuffs(long finalDommage, Fighter target, Fighter caster, Fight fight, int elementId , int idspell)
  {
      ...
      for(SpellEffect buff : target.getBuffsByEffectID(id))
      {
        switch(id)
        {
          ...
          case 9: //Derobade
            ...
            long newCellID=PathFinding.newCaseAfterPush(fight,caster.getCell(),target.getCell(),nbrCase,false);
            if(newCellID<0)
            {
              int a=(int) -newCellID;
              a=nbrCase-a;
          ...
          case 606://Chatiment (acncien)
        	  if(idspell == 435)
        		  break;
        	  long stat=buff.getValue();
            int jet=Formulas.getRandomJet(buff.getJet());
            target.addBuff((int) stat,jet,-1,-1,false,buff.getSpell(),buff.getArgs(),caster,true);
          ...
          case 788://Chatiments
        	  if(idspell == 435)
        		  break;
            long taux=(caster.getPersonnage()==null ? 1 : 2),gain=finalDommage/taux,max=0;
            stat=buff.getValue();
            ...
            {
              target.getChatiValue().put((int) stat,(int) newValue);
              target.addBuff((int) stat,gain,5,1,false,buff.getSpell(),buff.getArgs(),caster,true);
              SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight,7,stat,caster.getId()+"",target.getId()+","+gain+","+5);
            }
            target.getChatiValue().put((int) stat,(int) newValue);
            break;
    ...
    if(turns<=0)
    {
      ...
      for(Fighter target : cibles)
      {
        ...
        if(newCellId<0)
        {
          ...
          a=value-a;
          newCellId=(int) PathFinding.newCaseAfterPush(fight,caster.getCell(),target.getCell(),a,spell==73);
  ...
  private void applyEffect_6(ArrayList<Fighter> cibles, Fight fight)
  {
    if(turns<=0)
    {
      for(Fighter target : cibles)
      {
        ...
        int newCellID=(int) PathFinding.newCaseAfterPush(fight,eCell,target.getCell(),-value);
        if(newCellID==0)
          continue;

        if(newCellID<0)//S'il a ï¿½tï¿½ bloquï¿½
        {
          long a=-(value+newCellID);
          newCellID=(int) PathFinding.newCaseAfterPush(fight,caster.getCell(),target.getCell(),a);
          if(newCellID==0)
            continue;
          if(fight.getMap().getCase(newCellID)==null)
            continue;
  ...
  private void applyEffect_84(ArrayList<Fighter> cibles, Fight fight)
  {
    ...
    for(Fighter target : cibles)
    {
    	if(target.getMob() != null){
			...
			target.setCurPa(fight, (int) this.value);
  ...
  private void applyEffect_91(ArrayList<Fighter> cibles, Fight fight, boolean isCaC)//vole eau
  {
    if(isCaC)
    {
      for(Fighter target : cibles)
      {
        ...
        long finalDommage=Formulas.calculFinalDommage(fight,caster,target,Constant.ELEMENT_EAU,dmg,false,true,spell,this.cell,target.getCell(),aoe,isTrap);
    ...
    if(isCaC)
    {
      for(Fighter target : cibles)
      {
        ...
        else if(target.hasBuff(781)) //Jinx
        {
          ...
          dmg=Formulas.getRandomJet(args.split(";")[5]);
        long finalDommage=Formulas.calculFinalDommage(fight,caster,target,Constant.ELEMENT_TERRE,dmg,false,true,spell,this.cell,target.getCell(),aoe,isTrap);
    ...
    else if(turns<=0)
    {
      for(Fighter target : cibles)
      {
        ...
        else if(target.hasBuff(781)) //Jinx
        {
          ...
          dmg=Formulas.getRandomJet(args.split(";")[5]);
        long finalDommage=Formulas.calculFinalDommage(fight,caster,target,Constant.ELEMENT_TERRE,dmg,false,false,spell,this.cell,target.getCell(),aoe,false);
    ...
    if(isCaC)
    {
      for(Fighter target : cibles)
      {
        ...
        else if(target.hasBuff(781)) //Jinx
        {
          ...
          dmg=Formulas.getRandomJet(args.split(";")[5]);
        long finalDommage=Formulas.calculFinalDommage(fight,caster,target,Constant.ELEMENT_AIR,dmg,false,true,spell,this.cell,target.getCell(),aoe,isTrap);
    ...
    if(isCaC) //CaC Eau
    {
      for(Fighter target : cibles)
      {
        ...
        else if(target.hasBuff(781)) //Jinx
        {
          ...
          dmg=Formulas.getRandomJet(args.split(";")[5]);
        long finalDommage=Formulas.calculFinalDommage(fight,caster,target,Constant.ELEMENT_NEUTRE,dmg,false,true,spell,this.cell,target.getCell(),aoe,isTrap);
  ...
  private void applyEffect_101(ArrayList<Fighter> cibles, Fight fight)
  {
    if(spell==470)
    {
      for(Fighter target : cibles)
      {
        ...
        int retrait=Formulas.getPointsLost('a',(int) value,caster,target);
        //if(target.getCurPa(fight) <= 0)
        //	retrait = 0;
        if((value-retrait)>0)
    ...
    if(turns<=0)
    {
      for(Fighter target : cibles)
      {
        ...
        int remove=Formulas.getPointsLost('a',(int) value,caster,target);
        //if(target.getCurPa(fight) <= 0)
        //	remove = 0;
        if((value-remove)>0)
  ...
  private void applyEffect_109(Fight fight)//Dommage pour le lanceur (fixes)
  {
    if(turns<=0)
    {
      ...
      else if(caster.hasBuff(781)) //Jinx
      {
        ...
        dmg=Formulas.getRandomJet(args.split(";")[5]);
      long finalDommage=Formulas.calculFinalDommage(fight,caster,caster,Constant.ELEMENT_NULL,dmg,false,false,spell,this.cell,caster.getCell(),aoe,isTrap);
  ...
  private void applyEffect_127(ArrayList<Fighter> cibles, Fight fight)
  {
    if(turns<=0)
    {
      for(Fighter target : cibles)
      {
        int retrait=Formulas.getPointsLost('m',(int) value,caster,target);
        //if(target.getCurPm(fight) <= 0)
        //	retrait = 0;
        if((value-retrait)>0)
        {
      ...
      for(Fighter cible : cibles)
      {
    	  if(cible.getMob() != null){
				if(cible.getMob().getTemplate().getId() == 1071){
					...
					cible.setCurPa(fight, (int) this.value);
        ...
        if(fight.getFighterByOrdreJeu()==cible)
          fight.setCurFighterPa((int) (fight.getCurFighterPa()-value));
        if(cible.getMob()!=null)
  ...
  private void applyEffect_1003(ArrayList<Fighter> cibles, Fight fight)
  {
    ...
    for(Fighter target : cibles)
    {
      ...
      long val=(int)Math.floor((casterPdv/(float)100*(float)dmg)); //Valeur des dï¿½gats
  ...
  {
    long val=Formulas.getRandomJet(jet);
    if(val<0)
      return;
  ...
  private void applyEffect_1013(ArrayList<Fighter> cibles, Fight fight)
  {
    if(turns<=0)
    {
      for(Fighter target : cibles)
      {
        ...
        int newCellID=(int) PathFinding.newCaseAfterPush(fight,eCell,caster.getCell(),-value);
        if(newCellID==0)
          continue;

        if(newCellID<0) //S'il a ï¿½tï¿½ bloquï¿½
        {
          int a=(int) -(value+newCellID);
          newCellID=(int) PathFinding.newCaseAfterPush(fight,target.getCell(),caster.getCell(),a);
          if(newCellID==0)
            continue;
          if(fight.getMap().getCase(newCellID)==null)
            continue;
  ...
  public void verifmobs(Fight fight, Fighter target, int effet, long cura)
  {
