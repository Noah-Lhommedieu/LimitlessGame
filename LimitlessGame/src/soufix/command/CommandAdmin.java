...
public class CommandAdmin extends AdminUser
{
  ...
  public void command(String command, String[] infos, String msg)
  {
      ...
      else
      {
    	...
    	if(command.equalsIgnoreCase("EVENTGG"))
        {
          ...
          this.getPlayer().getAccount().setEvent( this.getPlayer().getAccount().getEvent()+1);
           String suffix="Le joueur "+perso.getName()+" vient de gagner 300 points boutique suite � l'event fait par "+this.getPlayer().getName()+" .";
    ...
    else if(command.equalsIgnoreCase("DELJOB"))
    {
      ...
      Database.getStatics().getPlayerData().update(perso);
      SocketManager.GAME_SEND_MESSAGE(perso,"Vous venez d'apprendre un m�tier, veuillez vous reconnecter pour finaliser les changements.");
    ...
    else if(command.equalsIgnoreCase("SHUTDOWN"))
    {
      ...
      else if(OffOn==1&&!this.isTimerStart())
      {
        if(time<=15)
        {
          for(Player player : Main.world.getOnlinePlayers())
          {
        	  SocketManager.PACKET_POPUP_DEPART(player, "Vous ne pouvez pas lancer de combats car le serveur red�marrera bient�t.");
            //player.sendServerMessage("Vous ne pouvez pas lancer de combats car le serveur red�marrera bient�t.");
            //player.send("M13");
          }
      ...
      else if(OffOn==0&&this.isTimerStart())
      {
        ...
        for(Player player : Main.world.getOnlinePlayers())
          player.sendServerMessage("Le red�marrage a �t� annul�. Vous pouvez recommencer les combats.");
        Config.getInstance().fightAsBlocked=false;
