...
public class CommandPlayerzoldik {
	private static String canal;

	static {
		CommandPlayerzoldik.canal = "Zoldik";
	}
		...
		if (msg.length() > 3 && msg.substring(1, 4).equalsIgnoreCase("all") || msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("monde")) {
			if (perso.noall) {
				SocketManager.GAME_SEND_MESSAGE(perso,
						"Votre canal " + CommandPlayerzoldik.canal + " est d\u00e9sactiv\u00e9.", "C35617");
				return true;
			}
			...
			if (msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("noall")) {
				if (perso.noall) {
					perso.noall = false;
					SocketManager.GAME_SEND_MESSAGE(perso,
							"Vous avez activ\u00e9 le canal " + CommandPlayerzoldik.canal + ".", "C35617");
				} else {
					perso.noall = true;
					SocketManager.GAME_SEND_MESSAGE(perso,
							"Vous avez d\u00e9sactiv\u00e9 le canal " + CommandPlayerzoldik.canal + ".", "C35617");
				}
				return true;
			}
			if (msg.length() > 9
			...
			if (msg.length() > 3 && msg.substring(1, 4).equalsIgnoreCase("vip")) {
					...
					SocketManager.PACKET_POPUP_DEPART(perso, 
							 "\n- Vos points acquis par vote sur Serveur-Prive passent à 35 PB par vote."
			...
			if (msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("event")) {
				...
				if (msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("infos")) {
					...
					int fake = 0;
					 ...
					 if(calcul>1200000) {
						 if(Config.getInstance().serverId == 8)
				        	fake =20;
					 }
