...
public class PlayerData extends AbstractDAO<Player>
{
  ...
  public boolean add(Player	 perso)
  {
    ...
    try
    {
      p=getPreparedStatement("INSERT INTO players(`id`, `name`, `sexe`, `class`, `color1`, `color2`, `color3`, `kamas`, `spellboost`, `capital`, `energy`, `level`, `xp`, `size`, `gfx`, `account`, `cell`, `map`, `spells`, `objets`, `storeObjets`, `morphMode`, `server`,`rapid`,`ornement`,`ornementsList`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'','','0',?,'','0',?)");
...
public void updateColor(int id, int color1, int color2, int color3)
{
  ...
  public void updatePrestige(Player perso)
  {
    PreparedStatement p=null;
    try
    {
      try
      {
    	  p=getPreparedStatement("UPDATE `players` SET `prestige` = ? WHERE `id`= ?");
          p.setInt(1,perso.getPrestige());
          p.setInt(2,perso.getId());
          execute(p);
      }
      catch(SQLException e)
    ...
    public void updateOmega(Player perso)
    {
      PreparedStatement p=null;
      try
      {
        try
        {
        	p=getPreparedStatement("UPDATE `players` SET `omega` = ? WHERE `id`= ?");
            int omegaId=(perso.getOmega());
            p.setInt(1,omegaId);
            p.setInt(2,perso.getId());
            execute(p);
          }
          ...
          {
            super.sendError("PlayerData updateOmega",e);
          } finally
          {
            close(p);
          }
    ...
    public void updateXpOmega(Player perso)
    {
      PreparedStatement p=null;
      try
      {
        try
        {
        	p=getPreparedStatement("UPDATE `players` SET `xpOmega` = ? WHERE `id`= ?");
            p.setDouble(1,perso.getxpOmega());
            p.setInt(2,perso.getId());
            execute(p);
          }
          catch(SQLException e)
          {
        	 super.sendError("PlayerData updateXpOmega",e);
          } finally
          {
            close(p);
          }
